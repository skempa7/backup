export const meta = { name: 'pharm-verify', description: 'Medically verify extracted pharmacology Preclaude questions', phases:[{title:'Verify'}] }
const INDIR = '/tmp/pharm_verify', OUTDIR = '/tmp/pharm_verify_out'
const LECS = Array.from({length:30}, (_,i)=>i+1)
log(`Verifying pharmacology: ${LECS.length} lectures`)
const prompt = (n) => String.raw`You are a rigorous board-certified clinical pharmacologist fact-checking exam questions for an NSU-KPCOM M1 pharmacology remediation quiz.
...
For EVERY question:
1) EXTRACTION INTEGRITY (fix silently): repair PDF-extraction artifacts — a truncated or stem-echoed explanation, a leaked "p. N LECTURE..." / "Q# LO x.y" fragment inside a choice or explanation, a giant study-doc dump appended to an explanation (common in the LAST question of a lecture — truncate it back to the real per-option reasoning), doubled spaces, mojibake/null-byte sequences (restore to '-'). Keep the SAME 5 options, same order, same correct index.
